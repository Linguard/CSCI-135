...
int main()
{
	...
	for(int row = 0; row < h; row++)
	{
		...
		{
			out[row + x][col + y] = img[row][col];
			out[row + x + 1][col + y] = img[row][col];
			out[row + x][col + y + 1] = img[row][col];
			out[row + x + 1][col + y + 1] = img[row][col];
	...
	writeImage(out, 2*h, 2*w);		// and save this new image to file "outImage.pgm"
