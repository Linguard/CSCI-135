...
draws a white frame exactly one pixel thick in the middle of the picture
and outputs the modified image to "outImage.pgm"
The dimensions of the frame should be 50% by 50% of the original picture’s width and height.
*/
...
int main()
{
	...
	for(int row = 0; row < h; row++)
	{
		...
		{
			out[row][col] =img[row][col];		//Normal picture
		}
	}
	...
	for(int row = h/4; row <= 3*h/4; row++)
	{
		...
		{
			if (row == h/4 or row == 3*h/4)
			{
				out[row][col] = 255;
			}

			if (col == w/4 or col == 3*w/4)
			{
				out[row][col] = 255;
			}
		}
	...
	writeImage(out, h, w);		// and save this new image to file "outImage.pgm"
